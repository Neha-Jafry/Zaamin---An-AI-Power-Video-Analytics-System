...
def draw_detections(frame, detections, palette, labels, output_transform,encoder,tracker):
    ...
    # bboxes = np.array(boxs)
    # scores = np.array(scores)
    # classes = np.array(classes)
    features = encoder(frame, bboxes)
    detections = [Detection(bbox, score, det_label, feature) for bbox, score, class_name, feature in zip(bboxes, scores, names, features)]
    ...
    for bbox in zones:
        print(bbox)
        cv2.rectangle(frame, (bbox["x"], bbox["y"]), (bbox["x"]+bbox["w"],bbox["y"]+ bbox["h"]), (0,125,0),2)
    for track in tracker.tracks:
        ...
        # draw bbox on screen
        cv2.rectangle(frame, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), palette[0], 2)
...
def main():
    ...
    while True:
        ...
        if results:
            ...
            frame = draw_detections(frame, objects, palette, model.labels, output_transform,encoder,tracker)
...
def test_video_feed():
    ...
    :return:
    """
    return Response(other(), mimetype='multipart/x-mixed-replace; boundary=frame')
